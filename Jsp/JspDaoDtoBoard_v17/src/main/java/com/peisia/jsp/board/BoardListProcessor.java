...
public class BoardListProcessor {
	...
	public BoardListProcessor(DaoBoard dao, String currentPage) {
		super();
		this.dao = dao;         //외부에서 DAO와 현재 페이지 번호를 넘겨 받아 초기화.
		this.currentPage = Integer.parseInt(currentPage);
		this.totalPage = getPageCount();   //내부에서 getPageCount()를 호출해 전체 페이지 수 계산.
		//todo
		//현재 페이지 번호와 전체 페이지 수를 기반으로
		getList();	//리스트 데이터 얻어오기

	}
	public void getList() {    //현재 페이지에 맞는 게시글 목록을 DAO를 통해 조회.
		//시작 인덱스 계산해서 넘기기

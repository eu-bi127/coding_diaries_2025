...
select * from tottenham_squad where p_position="공격수" order by weekly_pay desc;      #주급이 제일 높은 순부터.
select * from tottenham_squad order by p_number asc;
select * from tottenham_squad order by p_height desc, p_weight;

update tottenham_squad set weekly_pay = 1000 where p_number=9;           #등번호 9인 선수를 주급 1000으로 해놓기.

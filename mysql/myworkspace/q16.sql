...
select * from tottenham_squad where p_position="공격수" order by weekly_pay desc;
select * from tottenham_squad order by p_number asc;
select * from tottenham_squad order by p_height desc, p_weight;         # *는 전체라는 의미.

select p_number,p_name from tottenham_squad;                   # *을 넣어서 전체가 아니라, 선수단 정보 중 등번호와 이름만 뽑아서 보기
select p_number as 등번호,p_name as 선수명 from tottenham_squad;

update tottenham_squad set weekly_pay = 1000 where p_number=9;
update tottenham_squad set weekly_pay = 1000 where p_number=22;

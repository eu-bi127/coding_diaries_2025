...
public class Kiosk {
	...
	void run() {
		xx:while(true) {                   //while(true)를 사용해 프로그램을 계속 실행되도록 한다.
			System.out.print("명령 입력[1.음료선택/2.디저트선택/e.프로그램종료]:");
			cmd = sc.next();               //switch문을 통해 명령에 따라 다른 동작을 수행한다.
			switch(cmd) {
			case "1":
				yy:while(true) {
					//메뉴출력
...
//						Product x = new Product("아아",1000);
			...
			case "e":
				...
				int sum = 0;
				for(int i=0;i<basket.size();i=i+1) {    //for 루프를 사용하여 장바구니에 담긴 모든 상품의 가격을 합산.
